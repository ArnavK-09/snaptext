...
export default class SnapTextPreferences extends ExtensionPreferences {
  ...
  fillPreferencesWindow(window) {
    ...
    const descLabel = new Gtk.Label({
      label: _(
        "Instantly extract and copy text to your clipboard from anywhere on the screen.",
      ),
    ...
    const notificationRow = new Adw.ActionRow({
      ...
      subtitle: _(
        "Displays a system banner containing your copied text context.",
      ),
      title_lines: 0,
      subtitle_lines: 0,
    });
    ...
    const historyRow = new Adw.ActionRow({
      ...
      subtitle: _(
        "Keep a history of up to 15 recent extractions in the context menu.",
      ),
      title_lines: 0,
      subtitle_lines: 0,
    });
    ...
    const warningLabel = new Gtk.Label({
      label: _(
        "Auto-Translate sends extracted text to Google Translate. Privacy cannot be guaranteed when enabled.",
      ),
      wrap: true,
      xalign: 0,
      hexpand: true,
    ...
    const translateRow = new Adw.ActionRow({
      ...
      subtitle: _(
        "Automatically translate extracted text to your target language.",
      ),
      title_lines: 0,
      subtitle_lines: 0,
    });
    ...
    const langActionRow = new Adw.ActionRow({
      ...
      subtitle: _(
        "e.g., en, fr, es, zh. Leave empty to use your system default language.",
      ),
      title_lines: 0,
      subtitle_lines: 0,
    });
    ...
    const debugRow = new Adw.ActionRow({
      ...
      subtitle: _(
        "Outputs detailed OCR and process logs to the system journal.",
      ),
      title_lines: 0,
      subtitle_lines: 0,
    });
    ...
    const versionStr =
      this.metadata.version !== undefined
        ? this.metadata.version.toString()
        : "Local / EGO";
    ...
    linkBox.append(
      createLinkButton(
        _("Report a Bug \uD83D\uDC1E"),
        "https://github.com/cwittenberg/snaptext/issues/new?template=bug_report.md",
      ),
    );
    linkBox.append(
      createLinkButton(
        _("Request a Feature \uD83D\uDCA1"),
        "https://github.com/cwittenberg/snaptext/issues/new?template=feature_request.md",
      ),
    );
