...
function getCombinedInstallCommand(missingApps) {
  ...
  const distroLike = (GLib.get_os_info("ID_LIKE") || "")
    .toLowerCase()
    .split(/\s+/);
  ...
  if (isFedoraLike) {
    ...
    if (missingApps.includes("tesseract"))
      packages.push("tesseract tesseract-langpack-eng");
  ...
  if (isArchLike) {
    if (missingApps.includes("zbarimg")) packages.push("zbar");
    if (missingApps.includes("tesseract"))
      packages.push("tesseract tesseract-data-eng");
    if (missingApps.includes("imagemagick")) packages.push("imagemagick");
  ...
  if (isDebianLike) {
    ...
    if (missingApps.includes("tesseract"))
      packages.push("tesseract-ocr tesseract-ocr-eng");
    if (missingApps.includes("imagemagick")) packages.push("imagemagick");
  ...
  return (
    "# Install the missing dependencies using your distribution package manager:\n# " +
    missingApps.join(", ")
  );
}
...
const DependencyErrorDialog = GObject.registerClass(
  class DependencyErrorDialog extends ModalDialog.ModalDialog {
    _init(missingApps, installCmd) {
      ...
      let descText = _(
        "Please install the required packages and try again to enable all features.",
      );
      ...
      let tableBox = new St.BoxLayout({
        vertical: true,
        style:
          "spacing: 8px; background-color: rgba(128,128,128,0.1); padding: 12px; border-radius: 8px;",
      });
      ...
      let terminalInstruction = new St.Label({
        text: _(
          "Open a Terminal and run the following command to install the missing dependencies from the official repositories:",
        ),
      });
      ...
      let commandBox = new St.BoxLayout({
        vertical: false,
        style:
          "spacing: 12px; background-color: rgba(255,255,255,0.08); padding: 12px; border-radius: 8px;",
      });
      ...
      copyButton.connectObject(
        ...
        () => {
          ...
          this._copyIconTimeoutId = GLib.timeout_add(
